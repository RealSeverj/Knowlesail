    ...
    <div class="bg-background rounded-t-3xl p-4 max-h-[80vh] overflow-y-auto">
      ...
      <div class="space-y-3 text-sm">
        <div>
          ...
          <var-input v-model="title" placeholder="请输入待办名称" :maxlength="50" clearable />
        </div>
        <div>
          <label class="mb-1 block text-xs text-[var(--color-text-secondary)]">备注</label>
          <var-input v-model="note" placeholder="补充说明（可选）" type="textarea" rows="3" />
        </div>
        <div class="flex gap-3">
          <div class="flex-1">
            <label class="mb-1 block text-xs text-[var(--color-text-secondary)]">类别</label>
            <var-input v-model="category" placeholder="如 学习 / 工作" clearable />
            <div v-if="categoryOptions.length" class="mt-2 flex flex-wrap gap-2">
              <button
          ...
          <div class="flex-1">
            ...
            >
              <template #append-icon>
                <var-icon name="calendar" :size="18" color="var(--color-text-secondary)" />
              </template>
            </var-input>
          </div>
        </div>
      ...
      <div class="mt-4 flex gap-3">
        <var-button block text @click="handleClose"> 取消 </var-button>
        <var-button block type="primary" :disabled="!canSubmit" @click="handleSubmit">
          保存
        </var-button>
      </div>
    </div>
