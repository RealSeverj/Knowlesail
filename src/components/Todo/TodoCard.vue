...
<template>
  <div
    ...
    :class="[
      active ? 'h-full p-3' : compact ? 'w-36 p-2 opacity-95 hover:opacity-100' : 'p-3',
  ...
  >
    ...
    <div v-if="active" class="mt-2 flex-1 space-y-2 overflow-y-auto pb-2">
      <TodoItem v-for="todo in todos" :key="todo.id" :todo="todo" @edit="emit('edit-todo', todo)" />
      <p v-if="!todos.length" class="py-6 text-center text-xs text-[var(--color-text-secondary)]">
        这个象限还没有任务，去添加一个吧
      </p>
    </div>
  </div>
