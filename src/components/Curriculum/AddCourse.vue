    ...
    >
      <div class="bg-background rounded-t-3xl p-4 max-h-[80vh] overflow-y-auto">
        ...
        <div class="space-y-3 text-sm">
          ...
          <var-input v-model="form.name" placeholder="请输入课程名称" clearable></var-input>

          <label class="block text-xs text-[var(--color-text-secondary)]">任课教师</label>
          <var-input v-model="form.teacher" placeholder="例如：张三" clearable></var-input>

          <label class="block text-xs text-[var(--color-text-secondary)]">上课地点</label>
          <var-input v-model="form.location" placeholder="例如：西1-203" clearable> </var-input>
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              <var-input v-model.number="form.startWeek" type="number" placeholder="例：1" />
            </div>
            <div>
              <label class="block text-xs text-[var(--color-text-secondary)]">结束周</label>
              <var-input v-model.number="form.endWeek" type="number" placeholder="例：16" />
            </div>
          </div>
          ...
          <div class="mt-4 flex gap-3">
            <var-button text type="default" class="flex-1" @click="show = false"> 取消 </var-button>
            <var-button type="primary" class="flex-1" @click="handleSubmit"> 保存 </var-button>
          </div>
        </div>
      </div>
    </var-popup>
