...
<template>
	<div>
		<div
			class="absolute left-0.5 right-0.5 rounded-lg bg-primary/90 text-primary-foreground px-0.5 py-0.5 text-[11px] shadow cursor-pointer overflow-hidden flex flex-col justify-between border-2 border-opacity-40"
			:class="borderClass"
			:style="{ top: topStyle, height: heightStyle }"
			@click="showDetail = true"
		>
			<div class="flex-1 min-h-0 flex items-start">
				<div class="font-semibold leading-snug line-clamp-3 break-words pr-0 pl-0">
					{{ course.name }}
				</div>
			</div>
