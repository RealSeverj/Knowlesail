      ...
      <div class="rounded-xl bg-slate-50 dark:bg-slate-700/60">
        <div class="flex items-center justify-between px-3 py-2">
          ...
          <div class="relative">
            ...
            <PopMenu
              v-model="themeMenuVisible"
              :overlay="true"
              pop-class="absolute z-20 right-0 top-8"
              width-class="w-44"
            >
              <div class="space-y-1">
                <button
                ...
                >
                  ...
                  <var-icon v-if="theme?.id === item.value" name="check" :size="14" />
                </button>
    ...
    <section>
      ...
      <div class="overflow-hidden rounded-xl bg-slate-50 dark:bg-slate-700/60">
        <var-cell icon="cog-outline" title="偏好设置" ripple @click="onOpenPreference">
          <template #description>
            <span class="text-xs text-slate-500 dark:text-slate-400"> 笔记与学习体验相关设置 </span>
          </template>
          <template #extra>
    ...
    <section>
      ...
      <div class="overflow-hidden rounded-xl bg-slate-50 dark:bg-slate-700/60">
        <var-cell icon="information-outline" ripple title="关于学海智航" @click="onOpenAbout">
          <template #description>
            <span class="text-xs text-slate-500 dark:text-slate-400"> 版本信息与联系我们 </span>
          </template>
          <template #extra>
            <var-icon name="chevron-right" :size="18" />
          </template>
