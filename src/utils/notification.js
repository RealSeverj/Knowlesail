...
export async function cancelAllCourseRelatedNotifications() {
  ...
  console.log(
    '[notification] 已取消课程/待办/今日日程相关通知:',
    toCancel.map((n) => n.id)
  )
}
...
export async function scheduleNextClassReminder(nextClass) {
  ...
  if (nextClass.startTime) {
    startTime =
      nextClass.startTime instanceof Date ? nextClass.startTime : new Date(nextClass.startTime)
  } else if (nextClass.startTimeStr) {
    const now = new Date()
    const [h, m] = nextClass.startTimeStr.split(':').map(Number)
    startTime = new Date(now.getFullYear(), now.getMonth(), now.getDate(), h || 0, m || 0, 0, 0)
  }
...
export async function scheduleNearestTodoReminder(nearestTodo) {
  ...
  if (nearestTodo.deadline) {
    deadline =
      nearestTodo.deadline instanceof Date ? nearestTodo.deadline : new Date(nearestTodo.deadline)
